...
with open('file.mp3', 'rb') as f: # read in mp3 
    ...
    for i in range(numpacketsNeed):
        nPacket = int.to_bytes(i, SEQ_ID_SIZE, signed=True, byteorder='big') + f.read(MESSAGE_SIZE)
        packets_to_send.append(nPacket)
packets_to_send.append(int.to_bytes(-1, SEQ_ID_SIZE, signed=True, byteorder='big') )
